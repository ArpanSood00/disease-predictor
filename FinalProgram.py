...
import pickle
import joblib  # ✅ NEW: Best-practice model saving

# Your original list of models
all_models = [rf_classifier, logistic_model, dt_classifier, svc_classifier, knn_classifier]

# Original: Save all models using pickle (not recommended anymore)

with open("models.pkl", 'wb') as files:
    pickle.dump(all_models, files)


# ✅ NEW: Save models individually with joblib (recommended)
...
print("✅ Done saving models using joblib (best practice)")

# ✅ NEW: Optional – Save all in one dictionary for easy use
models_dict = {
    'knn': knn_classifier,
    'svc': svc_classifier,
...
print("✅ Done saving model dictionary with joblib")
...
# Original: Load from pickle
open_file = open("models.pkl", "rb")
loaded_list = pickle.load(open_file)
print(loaded_list)
open_file.close()
print("✅ Done loading with pickle")

# ✅ NEW: Load models using joblib (recommended way)
...
logistic_loaded = joblib.load('logistic_model.joblib')
print("✅ Done loading individual models using joblib")

# ✅ NEW: Load model dictionary (optional convenience)
loaded_models_dict = joblib.load('all_models.joblib')
print("✅ Loaded model keys:", loaded_models_dict.keys())
